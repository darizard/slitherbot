...
export async function validateAndRefreshUserAccessTokens(tokens: string | string[] | null = null): Promise<void> {
    ...
    if(tokensToUpsert.length > 0) {
        const result = await twitchsql.upsertUsers(tokensToUpsert)
        console.log(`Refreshed tokens with ${result.length} InsertResults`)
    }
    if(channelIDsToClear.length > 0) {
        const result = await clearTwitchUserAccessTokens(channelIDsToClear)
        const results_arr: number[] = []
        for(let result_el of result) results_arr.push(Number(result_el.numUpdatedRows))
        console.log(`Handled invalid tokens with UpdateResult[]: ${JSON.stringify(results_arr)}`)
    }
}
    ...
    for(let token of tokens) {
        await fetch(new Request(`https://id.twitch.tv/oauth2/validate`, { method: 'GET',
            ...
            if(isTwitchAuthTokenValidationResponse(twitchResponse as TwitchAuthTokenValidationResponse)) { 
                console.log(`Validated token for channel_id: ${twitchResponse.user_id}`)
                return 
            }
            else if(isTwitchAuthTokenValidationErrorResponse(twitchResponse as TwitchAuthTokenValidationErrorResponse)) {
                console.log(`Encountered invalid token for channel_id: ${token.channel_id}`)
...
export async function clearTwitchUserAccessTokens(channel_ids: string[] | string) {
    ...
    const updateResults = await twitchsql.invalidateUserAccessTokens(typeof channel_ids === 'string' ? [channel_ids] : channel_ids)
...
export async function oauthStateOrParamProblem(auth_states: string[], req: Request, res: Response): Promise<boolean> {
...
export async function fetchTwitchUserAccessToken(req: Request): Promise<TwitchAuthUserToken> {
    ...
    // Just make sure the response payload is as expected
    ...
    return twitchTokenData
}

export async function validateTokenAndGetUserID(access_token: string): Promise<string> {
    await fetch(new Request(`https://id.twitch.tv/oauth2/validate`,
                                { method: 'GET',
                                  headers: 
                                    { Authorization: `OAuth ${access_token}` } }
    )).then(async (res) => {

        return (await res.json() as TwitchAuthTokenValidationResponse).user_id
